...
class ForensicCheckpointAlertRequest(BaseModel):
    ...
    rule:     str = Field(
        ...
        json_schema_extra={"example": "Terminal shell in container"}
    )
    priority: str = Field(
        description="Severity level of the alert.",
        json_schema_extra={"example": "Notice"}
    )
    output:   str = Field( 
        description="Human-readable output message generated by Falco."
    )
    output_fields: Optional[dict] = Field(
        default=None, 
        description="Key-value pairs of specific fields extracted by the Falco engine (e.g., container.id, proc.name)."
    )
    tags:     Optional[list[str]] = Field(
        default=None, 
        description="Metadata tags associated with the Falco rule.",
        json_schema_extra={"example": ["container", "shell", "mitre_execution"]}
    )
