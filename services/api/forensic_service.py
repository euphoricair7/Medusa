...
async def find_recent_duplicate(session: AsyncSession, key: str) -> ForensicEvent | None:
...
async def process_trigger_forensic(
    ...
    *,
    ...
    idem_key = make_idempotency_key(namespace, pod_name, rule, settings.idempotency_window_seconds)

    existing = await find_recent_duplicate(session, idem_key)
    if existing:
        return existing
